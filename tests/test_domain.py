...
def test_wildcard_inside_strict(run_dir):
    ...
    # (a known engine difference), so this gdxcc-specific behavior is pinned here.
    out = os.path.join(run_dir, "wildcard_inside_strict.gdx")
    with GdxFile(engine="gdxcc") as gdx:
    ...
    with GdxFile(lazy_load=False, engine="gdxcc") as gdx:
        gdx.read(out)
        mix = gdx["mix"]
