"""Read/write parity between the gdxcc and gams_transfer engines: both must
...
# not), so it can't satisfy strict cross-engine parity. It is covered separately
...
def _assert_same(a, b):
    # Same symbols in the same order (both engines read in GDX order), then
...
def test_read_parity(data_dir, fixture):
    ...
    a = to_dataframes(path, engine="gdxcc")
    b = to_dataframes(path, engine="gams_transfer")
    _assert_same(a, b)
...
def test_read_parity_special_values():
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        to_gdx({"p": df}, out)
        a = to_dataframes(out, engine="gdxcc")
        b = to_dataframes(out, engine="gams_transfer")
        _assert_same(a, b)
...
def test_read_parity_undef():
    ...
    # then assert both engines read it back identically (object col, None vs nan).
...
def test_read_parity_symbol_subset(data_dir):
    ...
    names = list(to_dataframes(path, engine="gdxcc"))[:2]
    a = to_dataframes(path, engine="gdxcc", symbols=names)
    b = to_dataframes(path, engine="gams_transfer", symbols=names)
    assert list(a) == names and list(b) == names
    _assert_same(a, b)


# --- Phase B: write parity, over the full write x read engine matrix ---

_ENGINES = ("gdxcc", "gams_transfer")


def _write_read_matrix(dfs, tmp_path, **kw):
    """Write ``dfs`` with each engine, then read each output back with each
    engine. Returns ``{(write_engine, read_engine): dataframes}``, exercising
    ...
    for w in _ENGINES:
        ...
        to_gdx(dfs, paths[w], engine=w, **kw)
    return {(w, r): to_dataframes(paths[w], engine=r) for w in _ENGINES for r in _ENGINES}
...
def test_write_parity(data_dir, fixture, tmp_path):
    dfs = to_dataframes(os.path.join(data_dir, fixture), engine="gdxcc")
...
def test_write_parity_undef(tmp_path):
    ...
    # NA (np.nan). Both engines preserve the distinction on write: UNDEF round-trips
...
@pytest.mark.parametrize("engine", ["gdxcc", "gams_transfer"])
def test_write_alias_via_to_gdx(tmp_path, engine):
    # to_gdx(aliases=) builds an alias of a Set given as a DataFrame, on both engines.
    dfs = {"t": pd.DataFrame({"i": ["a", "b", "c"], "Value": ["", "", ""]})}
    out = str(tmp_path / "al.gdx")
    to_gdx(dfs, out, engine=engine, aliases={"at": "t"})
    with GdxFile(lazy_load=False, engine=engine) as g:
        g.read(out)
