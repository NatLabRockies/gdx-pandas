...
def test_gdxfile_cleanup_is_idempotent():
    ...
    f = gdxpds.gdx.GdxFile(backend="gdxcc")
    assert f._finalizer.alive  # handle live, not yet freed
    f.cleanup()
    assert not f._finalizer.alive  # freed, exactly once
    assert f._backend_impl.handle is None  # gdxcc handle freed/cleared on close
...
def test_gdxfile_H_removed():
    # GdxFile.H is gone in v3.0.0; the raw gdxcc handle is reached via the backend.
    f = gdxpds.gdx.GdxFile(backend="gdxcc")
    assert not hasattr(f, "H")
    assert f._backend_impl.handle is not None  # the documented escape hatch
    f.cleanup()
    assert f._backend_impl.handle is None
...
def test_many_gdxfiles_exit_cleanly():
    ...
    code = (
        ...
        "for _ in range(50):\n"
        "    g = gdxpds.gdx.GdxFile(backend='gdxcc'); g.cleanup()\n"
        "fs = [gdxpds.gdx.GdxFile(backend='gdxcc') for _ in range(50)]\n"
        "del fs; gc.collect()\n"
    )
...
def test_invalid_gams_dir_fails_cleanly():
    ...
    result = subprocess.run(
        [sys.executable, "-c", "import gdxpds.gdx; gdxpds.gdx.GdxFile(backend='gdxcc')"],
        capture_output=True,
        text=True,
        env=env,
    )
