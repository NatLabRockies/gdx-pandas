...
This package **cannot function without a GAMS installation** — there is no mock layer. The Python package `gdxcc` (official GAMS SWIG bindings) is imported at module load and talks to the GAMS shared library found at runtime.

- GAMS lookup order is implemented by `GamsDirFinder` in [gdxpds/tools.py](gdxpds/tools.py): `GAMS_DIR` env var → `which gams` (POSIX) → `C:/GAMS` (Windows, highest version).
...
- `gdxcc` itself usually needs a manual install step from `GAMS/*/apifiles/Python/api_XX/` — `pip install gdxcc` may not be enough. See [README.md](README.md) for the `python setup.py install` dance and the `_*.pyd`/`_*.so` copy workaround.

If tests fail with "cannot load gdxcc" or "no `_gdxcc` module," it's a GAMS environment problem, not a code bug.
...
- **Optional fast path.** If `gdx2py` is importable (Windows-only), it is used to read Parameters faster than streaming through `gdxcc`. The slow path through `gdxcc.gdxDataReadStr()` is the default.
...
- Test fixtures include real `.gdx` and `.csv` files in [gdxpds/test/](gdxpds/test/); these are packaged via `package_data` in [setup.py](setup.py). Don't delete them.
- The two CLI scripts in [bin/](bin/) (`csv_to_gdx.py`, `gdx_to_csv.py`) are installed as scripts by setuptools — keep them runnable as `__main__`.
